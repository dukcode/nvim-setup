...
### nvim-tree 내부 (트리에 포커스 있을 때)

기본 매핑(`default_on_attach`)을 그대로 쓴다. 도움말은 트리에서 `g?`.
...
| `t` | 새 탭으로 열기 (포커스 이동) |
| `T` | 새 탭으로 열되 트리에 머무름 |
| `<C-v>` / `<C-x>` | 수직 / 수평 split으로 열기 |
| `<Tab>` | 미리보기 (포커스 트리에 유지) |
| `<BS>` | 부모 폴더 닫기 |
| `<Esc>` | 트리 유지, 직전 창으로 포커스 (커스텀) |
...
| `K` / `J` | 첫 / 마지막 형제 |
| `[c` / `]c` | 이전 / 다음 git 변경 |
| `[e` / `]e` | 이전 / 다음 diagnostic |
| `<C-]>` | 커서 폴더를 루트로 (cd into) |
| `-` | 한 단계 상위를 루트로 |
...
|---|---|
| `a` | 생성 (`이름/`이면 폴더) |
| `r` / `e` | 이름 변경 (전체 / basename만) |
| `d` / `D` | 삭제 / trash로 |
| `x` / `c` / `p` | 잘라내기 / 복사 / 붙여넣기 |
| `y` / `Y` / `gy` | 파일명 / 상대경로 / 절대경로 복사 |
